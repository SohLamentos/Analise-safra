...
import { parseExcelPercent, cn, normalizeNumber, normalizePercent } from '../lib/utils';
...
export default function ImportSafra({ data, setData, config }: ImportSafraProps) {
  ...
  const [manualType, setManualType] = useState<'SAFRA' | 'VETERANO' | 'VETERANO_EM_SAFRA'>('SAFRA');
      ...
      try {
        ...
        wb.SheetNames.forEach(sheetName => {
          ...
          const sheetRecords = rawRows.map(row => {
            try {
              const rev = normalizePercent(row['% REV.']);
              const agenda = normalizePercent(row['% CUMP. AGENDA']);
              const gapProd = normalizeNumber(row['GAP PROD. (PONTOS)']);
              const gapRev = normalizeNumber(row['GAP REV.']);
              const gapAgenda = normalizeNumber(row['GAP CUMP. AGENDA (OSs)']);
              const mediaDias = normalizeNumber(row['MÉDIA DIAS TRABALHADOS']);
              ...
              const percentTC = normalizePercent(row['% TC']);
              let isCertified = percentTC >= 70;
              ...
              if (percentTC === 100) {
                ...
                if (normalizeText(row['SEM EAD']) === 'SIM' || (normalizeNumber(row['GAP EAD'])) > 0) reasons.push('SEM EAD');
                if (normalizeText(row['NOSHOW']) === 'SIM' || (normalizeNumber(row['GAP NOSHOW'])) > 0) reasons.push('NOSHOW');

                classification = percentTC >= 70 ? 'CERTIFICADO' : (percentTC > 0 ? 'QUASE CERTIFICADO' : 'NÃO CERTIFICADO');
                score = percentTC || 0;
              }
              ...
              let baseFinal: 'SAFRA' | 'VETERANO' | 'VETERANO_EM_SAFRA' = manualType;

              if (tipoPlanilhaRaw === 'SAFRA' || tipoPlanilhaRaw === 'VETERANO' || tipoPlanilhaRaw === 'VETERANO_EM_SAFRA') {
                baseFinal = tipoPlanilhaRaw as 'SAFRA' | 'VETERANO' | 'VETERANO_EM_SAFRA';
              }
              ...
              return {
                ...
                QTDE_WOS: qtdeWOS,
                QTDE_OSS: qtdeOSS,
                QTDE_REV: normalizeNumber(row['QTDE REV.']),
                PROD: normalizeNumber(row['PROD.']),
                GAP_PROD: gapProd,
                PERCENT_REV: rev,
                GAP_REV: gapRev,
        ...
        setSummary({
          ...
          veteranoEmSafra: finalProcessed.filter(r => r.TIPO_BASE === 'VETERANO_EM_SAFRA').length,
      ...
      <div className="card p-12 text-center relative overflow-hidden group">
        ...
        <div className="flex flex-col items-center gap-6 mb-10">
           <div className="flex gap-3">
              ...
              <button 
                onClick={() => setManualType('VETERANO_EM_SAFRA')}
                className={cn(
                  "px-5 py-2.5 rounded-xl font-bold text-xs border transition-all",
                  manualType === 'VETERANO_EM_SAFRA' ? "border-primary bg-primary/5 text-primary" : "border-slate-200 text-slate-400 hover:border-slate-300"
                )}
              >
                Importar como VETERANO EM SAFRA
              </button>
           </div>
        </div>
        ...
        {summary && (
          ...
          >
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6 mt-6 pt-6 border-t border-green-200">
               ...
               <div className="bg-white/50 p-3 rounded-xl border border-green-100">
                  <div className="text-[10px] font-bold text-indigo-800 uppercase mb-1 tracking-tighter text-center font-mono">VET EM SAFRA</div>
                  <div className="text-xl font-black text-indigo-900 text-center">{summary.veteranoEmSafra}</div>
               </div>
               <div className="bg-white/50 p-3 rounded-xl border border-green-100">
