...
import SafraCriticos from './components/SafraCriticos';
import Partners from './components/Partners';
import MonthlyEvolution from './components/MonthlyEvolution';
import TechnicianEvolution from './components/TechnicianEvolution';
import Settings from './components/Settings';
import ImportSafra from './components/ImportSafra';

// New Decision Views
import CriticosView from './components/CriticosView';
import RecuperaveisView from './components/RecuperaveisView';
...
export default function App() {
  ...
  const [selectedTechLogin, setSelectedTechLogin] = useState<string | null>(null);
  const [isSidebarOpen, setIsSidebarOpen] = useState(true);
  const [isAdminMenuOpen, setIsAdminMenuOpen] = useState(false);
  const [selectedRegion, setSelectedRegion] = useState<'TODAS' | 'INTERIOR' | 'CAPITAL' | 'NAO_CLASSIFICADO'>('TODAS');
  const [selectedType, setSelectedType] = useState<'TODAS' | 'SAFRA' | 'VETERANO'>('TODAS');
  ...
  const mainNavItems = [
    ...
    { id: 'recuperaveis', label: 'Recuperáveis', icon: TrendingUp },
    { id: 'safraContinuity', label: 'Continuidade da Safra', icon: Users },
    { id: 'techEvolution', label: 'Evolução do Técnico', icon: History },
    { id: 'partners', label: 'Empresas', icon: Building2 },
    { id: 'transicao', label: 'Transição Safra x Veterano', icon: LineChartIcon },
  ...
  const renderContent = () => {
    ...
    const handleViewTech = (login: string) => {
      setSelectedTechLogin(login);
      setActiveView('techEvolution');
    }

    try {
      switch (activeView) {
        case 'dashboard': return <Dashboard data={filteredData} config={config} />;
        case 'criticos': return <CriticosView data={filteredData} config={config} />;
        case 'recuperaveis': return <RecuperaveisView data={filteredData} config={config} />;
        case 'techEvolution': return <TechnicianEvolution data={filteredData} config={config} initialLogin={selectedTechLogin} />;
        ...
        case 'safraContinuity': return <SafraContinuity data={data} config={config} onViewTech={handleViewTech} />;
      ...
      <main className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        <header className="h-16 bg-white border-b border-slate-200 px-6 flex items-center justify-between shrink-0">
          ...
          <div className="hidden lg:flex items-center gap-6">
            ...
            {/* Type Filter */}
            <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-xl">
              {(['TODAS', 'SAFRA', 'VETERANO'] as const).map(t => (
                <button
                  key={t}
